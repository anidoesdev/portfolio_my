...
2. **Hero** (`#hero`) — a **plate**, ranged left over the full-bleed meadow under its three-layer scrim (cream fade at the foot, a centre pool for text contrast, then the flat dim). Role, name, one line of lede, then a `<dl>` of fields: **Field**, **Stack**, **Source**.
   **Fields rather than sentences.** The hero is the one place a reader has no context yet — `AI Engineer` answers a question where a line about being passionate about building answers none. Every value on the plate is already stated further down the page: the four fields are the four project subjects, the stack entries are in Skills, the handle is the one Contributions reads from. Nothing here is a claim the rest of the site does not already make and let a reader check.
   Hairlines and a fixed 6rem label column, the **same setting as the Skills spec sheet** — those are the page's only two tabular blocks and they should read as the same kind of object. A `<dl>`, so each label is bound to its value rather than the two happening to share a line.
   **Ranged left, not centred.** A centred stack is the hero layout every portfolio already has, and left alignment is what lets the plate read as a column of values instead of six lines of poetry. The three jump keys take `--font-crt` like every other control, and the **five arrows are gone** — one had sat inside each key saying what its label already said, with a fourth bouncing on a loop underneath saying it a fifth time
3. **Projects** (`#projects`) — **folder tabs**: name tags along the top, one folder body below, each tab carrying a pip that **pulses when that project is deployed** (1.6s cycle, dipping to 0.35 rather than going out — a light that extinguishes reads as a fault; steady under `prefers-reduced-motion`, and well under the three-per-second flashing ceiling). The body is a two-column grid: architecture diagram left, text right, description **justified with `hyphens: auto`** — mandatory at a ~55-character measure, or justification opens rivers of white. Actions are two keys rather than the same key twice: `Source` always, `Launch` **lit** and rendered only when something is actually deployed, both with icons drawn in the schematic's straight-segment language and stroked in `currentColor` so they track the label through every state. The stage stepper is a **two-position rocker**: `Flow` and `Demo` either side of a recessed track with a knurled knob that travels between them, so the knob-s place *is* the state and both labels are readable at rest. It replaced a hover panel of thumbnails, which made the reader hover *and then look* to work out what a dot did; the readout is `aria-hidden` and holds no controls, since the dots already carry the full stage names on their `aria-label`s. The jump from diagram to demo now runs **once per folder**, the first time it is open and on screen, rather than re-arming on every replay: a diagram you had gone back to read would otherwise take itself away while you were reading it. The progress line under the stage went with it — a permanent bar advertising a one-time event is more chrome than the event is worth, and the `sr-only` live region still announces the change. Full spec, tuning notes and invariants live in `src/components/Projects.design.md`
