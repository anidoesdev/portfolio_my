...
3. **Projects** (`#projects`) — **folder tabs**: name tags along the top, one folder body below, each tab carrying a pip that **pulses when that project is deployed** (1.6s cycle, dipping to 0.35 rather than going out — a light that extinguishes reads as a fault; steady under `prefers-reduced-motion`, and well under the three-per-second flashing ceiling). The body is a two-column grid: architecture diagram left, text right, description **justified with `hyphens: auto`** — mandatory at a ~55-character measure, or justification opens rivers of white. Actions are two keys rather than the same key twice: `Source` always, `Launch` **lit** and rendered only when something is actually deployed, both with icons drawn in the schematic's straight-segment language and stroked in `currentColor` so they track the label through every state. The stage dots open **one panel showing both stages side by side**, on hover *and* focus: a still `Schematic` for the diagram beside the YouTube poster frame for the demo, with the stage you are on marked rather than hidden — which answers whether advancing is worth it without paying for an iframe to find out. That needed a `still` prop on `Schematic` (no observer, never arms, `aria-hidden` since the live diagram already carries the alt text) and moved the arrowhead marker id to a sanitised `useId()`: it had been keyed on viewBox size, which held only because all four diagrams differ in height and stopped holding the moment a still copy was drawn beside one. Full spec, tuning notes and invariants live in `src/components/Projects.design.md`
